...
def main():
	...
	if read != crc_blk(b'\x9f\x34\x56\x78'):
		print('Error: Check connection to the floader!')
		serialPort.close
		sys.exit(2)
	print('------------------------------------------------')
	...
	if args.operation == 'rf':
		serialPort.timeout = MAX_RXTX_BLOCK_SIZE*12/args.baud + 0.001
		...
		stream.close
		if t > 0:
			throughPut = args.size/t;
			print("Throughput: %d Bytes/s " % throughPut);
